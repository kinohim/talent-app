...
import { getSkillDashboard } from "@/lib/dashboard";

/**
 * GET /api/dashboard/skills?departmentId= — スキルマップ／組織ダッシュボード（REF008）。
...
export async function GET(req: NextRequest) {
  ...
  const items = await getSkillDashboard(session, departmentId);
